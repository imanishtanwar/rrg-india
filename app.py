...
if benchmark_df is None:
    st.error("Benchmark data could not be loaded. Please refresh.")
    st.stop()
...
# -----------------------------
# -----------------------------
# USE ONLY AVAILABLE SECTORS
# -----------------------------
available_sectors = [s for s in sectors.keys() if s in prices.columns]

if len(available_sectors) < 2:
    st.error("Not enough sector data available to compute RRG. Please refresh.")
    st.stop()

rs = prices[available_sectors].div(prices["Benchmark"], axis=0)
